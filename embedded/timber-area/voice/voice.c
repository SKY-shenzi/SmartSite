...
extern int32_t distance;   /* app_main.c */
extern int lampState;       /* 0=ON, 1=OFF */
extern int fanState;        /* 0=ON, 1=OFF */
extern int buzzerState;     /* 0=ON, 1=OFF */
extern uint16_t co2_value;  /* CO2 PPM */
extern uint16_t mq2_value;  /* 烟雾 0-100% */

// 宏定义配置
#define DELAY_TIME_MS 10
#define UART_RECV_SIZE 14
#define UART_INT_MODE 0          /* 轮询模式（参照 25_smart_home） */
...
void voice_analysis(uint8_t *info)
{
    ...
    switch (temp) {
        ...
        case 0x03: // 打开风扇
            printf("Voice cmd: Turn on Fan\r\n");
            fanState = 0;
            fan_on();
            break;

        case 0x04: // 关闭风扇
            printf("Voice cmd: Turn off Fan\r\n");
            fanState = 1;
            fan_off();
            break;

        case 0x10: // 打开蜂鸣器
            printf("Voice cmd: Turn on Buzzer\r\n");
            buzzerState = 0;
            beep_on();
            break;

        case 0x11: // 关闭蜂鸣器
            printf("Voice cmd: Turn off Buzzer\r\n");
            buzzerState = 1;
            beep_off();
            break;
        ...
        case 0x0B: // 询问当前湿度
            printf("Voice cmd: Query humidity\r\n");
            {
                ...
                if (dht11_read_data(&dht_humi_data) == ERRCODE_SUCC) {
                    ...
                    usart_su_TXpacket[3] = dht_humi_data.humi_high8bit;
                    usart_su_TXpacket[4] = 0x00;
                    usart_su_TXpacket[5] = 0x00;
                    usart_su_TXpacket[6] = 0x00;
                    usart_su_TXpacket[7] = dht_humi_data.humi_low8bit;
                    usart_su_TXpacket[8] = 0x00;
                    usart_su_TXpacket[9] = 0x00;
                    usart_su_TXpacket[10] = 0x00;
                    usart_su_TXpacket[11] = 0X55;
                    ...
                    printf("Read DHT11 humidity failed!\n");
                }
            }
            break;
        ...
        case 0x0C: // 询问当前距离
            ...
            if (distance > 0) {
                ...
                usart_su_TXpacket[3] = distance & 0xFF;
                usart_su_TXpacket[4] = (distance >> 8) & 0xFF;
                usart_su_TXpacket[5] = 0x00;
                usart_su_TXpacket[6] = 0x00;
        ...
        case 0x0D: // 询问CO2浓度
            printf("Voice cmd: Query CO2\r\n");
            co2_value = co2_get_eco2();
            if (co2_value > 0) {
                usart_su_TXpacket[0] = 0XAA;
                usart_su_TXpacket[1] = 0X55;
                usart_su_TXpacket[2] = 0x04;
                usart_su_TXpacket[3] = co2_value & 0xFF;
                usart_su_TXpacket[4] = (co2_value >> 8) & 0xFF;
                usart_su_TXpacket[5] = 0x00;
                usart_su_TXpacket[6] = 0x00;
                usart_su_TXpacket[7] = 0X55;
                usart_su_TXpacket[8] = 0XAA;
                uapi_uart_write(UART_BUS_2, usart_su_TXpacket, 9, 0);
                printf("Send CO2: %d PPM\n", co2_value);
            } else {
                printf("Read CO2 failed!\n");
            }
            break;
        ...
        case 0x0E: // 询问烟雾浓度
            ...
            usart_su_TXpacket[3] = mq2_value & 0xFF;
            usart_su_TXpacket[4] = (mq2_value >> 8) & 0xFF;
            usart_su_TXpacket[5] = 0x00;
            usart_su_TXpacket[6] = 0x00;
            usart_su_TXpacket[7] = 0X55;
            usart_su_TXpacket[8] = 0XAA;
